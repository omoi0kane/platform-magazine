...
The source is treated as read-only. Generated pages are JPEG, preserve aspect ratio, and use manifest-controlled `max_dimension` and `jpeg_quality`. For an image directory, declare the cover explicitly. `affinity_spreads` converts separately exported Affinity pages into Udon Magazine's historical `3,2,5,4...` content order. `affinity_spread_pages` handles a PDF whose interior PDF pages are already horizontal spreads: it renders each half at full per-page resolution and emits the right half followed by the left half. `explicit` accepts an exact filename list. The cover is used only by the cover material and is excluded from `pageTextures`.

For a PDF exported as front cover, interior pages, and back cover, set `source.back_cover: true`. Matching the established Platform packages, the builder combines PDF page 1 and the final PDF page into a horizontal cover atlas (front on the left, back on the right) for the cover material. `pageTextures` then contains only the interior in the declared runtime order. No blank page is inserted.
